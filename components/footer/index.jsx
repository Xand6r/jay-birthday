...
import { isLoggedIn, logIn } from "utils/functions";
import { useEffect } from "react";
import { postRequest } from "api";

export default function Footer() {
  const password = "bigbaddiejay";
  var validRegex =
    /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/;

  const loginUser = () => {
    if (textContent === password) {
      toast.success("Correct Password, logging you in");
      logIn();
  ...
  const rsvp = async () => {
    ...
    try {
      const { data: response } = await postRequest("/rsvp", {
        email: textContent,
      });
...
const InstagramIcon = () => (
  <span
    ...
    onClick={() => window.open("https://instagram.com/jacintaamune", "_blank")}
  >
    <svg
      width="20"
      height="20"
